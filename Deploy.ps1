# NASDY Media Linker - Deploy v3.0.1 Cleanup
# One-command deployment with SSH-key support, local backup, source packaging,
# raw LF-safe remote script execution, NAS-side Docker build/restart,
# /host_mnt verification, and /health verification.
...
$BackupPath = Join-Path $BackupDir "deploy-v3.0.1-local-backup-$Stamp.tar.gz"
$script:UseKey = $false
...
function Invoke-SshScriptFile {
    ...
    if (-not (Test-Path $ScriptPath)) {
        throw "SSH script file does not exist: $ScriptPath"
    }
    ...
    if (-not $process.Start()) {
        throw "Could not start ssh process."
    }

    $inputStream = $null
    try {
        $inputStream = [System.IO.File]::OpenRead($ScriptPath)
...
function Normalize-DeployShellScript {
    $deployShellPath = Join-Path $ProjectRoot ".deploy.sh"
    if (-not (Test-Path $deployShellPath)) {
        throw ".deploy.sh was not found in $ProjectRoot. Run the Deploy v3.0.1 cleanup upgrade first."
    }

    $current = [System.IO.File]::ReadAllText($deployShellPath)
    $normalized = ConvertTo-LfText $current
    if ($normalized -ne $current) {
        Write-Info "Normalizing .deploy.sh to Unix LF line endings."
        Write-Utf8NoBomLf -Path $deployShellPath -Text $normalized
    }
}
...
try {
    ...
    if (Test-SshKeyWorks) {
        ...
        Write-DeployWarning "SSH key was not available or did not pass key-only testing. Falling back to password SSH for this deploy."
        Write-DeployWarning "Run .\Setup-NASDY-SSHKey.ps1 to enable zero-prompt deploys."
    }

    Normalize-DeployShellScript
    Remove-PythonCaches
...
if [ -f .deploy.sh ]; then
  tr -d '\015' < .deploy.sh > .deploy.sh.lf
  mv .deploy.sh.lf .deploy.sh
  chmod +x .deploy.sh
else
  printf '%s\n' "ERROR: .deploy.sh was not found after source sync." >&2
  exit 1
fi
...
'@
    ...
    Write-Utf8NoBomLf -Path $RemoteScriptPath -Text ($remoteScript + "`n")

    $sshArgs = Get-SshArgs
    $sshArgs += @($Remote, "sh", "-s")

    Write-Info "Opening LF-safe deployment SSH session to $Remote."
    $remoteCode = Invoke-SshScriptFile -Arguments $sshArgs -ScriptPath $RemoteScriptPath
    if ($remoteCode -ne 0) {
    ...
    Write-Host " NASDY Deploy v3.0.1 Complete" -ForegroundColor Green
    Write-Host "====================================" -ForegroundColor Green
    Write-Good "SSH authentication path selected"
    Write-Good "Source synced"
    Write-Good "Docker built and container restarted"
    Write-Good "/host_mnt verified"
    Write-Good "/health verified"
