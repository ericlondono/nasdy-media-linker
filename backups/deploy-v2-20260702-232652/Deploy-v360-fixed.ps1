...
$RemoteVersion = ssh "$NasUser@$NasHost" "python3 - <<'PY'
import re
from pathlib import Path
text = Path('$RemoteProjectPath/app/config.py').read_text(encoding='utf-8')
match = re.search(r'APP_VERSION\s*=\s*[\"'\"']([^\"'\"']+)[\"'\"']', text)
print(match.group(1) if match else '')
PY"

$RemoteVersion = (($RemoteVersion | Where-Object { $_ -and $_.Trim() } | Select-Object -Last 1) -as [string])
if ($RemoteVersion) {
