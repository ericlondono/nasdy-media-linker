...
$backup = Join-Path $ProjectRoot "backup-before-v3472-$stamp"
...
$config = [regex]::Replace($config, 'APP_VERSION\s*=\s*"[^"]+"', 'APP_VERSION = "v3.4.7.2"')
Set-Content $configPath $config -Encoding UTF8

# Replace app.js with a known-good version that owns all queue filtering and avoids inline duplicate scripts.
...
# Remove all duplicate inline queue/history scripts from index.html and leave only the external app.js.
$indexPath = Join-Path $ProjectRoot "app\templates\index.html"
$index = Get-Content $indexPath -Raw -Encoding UTF8
$index = [regex]::Replace($index, 'APP_VERSION\s*=\s*"[^"]+"', 'APP_VERSION = "v3.4.7.2"')
# Remove any script block after the app.js include and before body close.
$index = [regex]::Replace($index, '(?s)<script>\s*\(function \(\).*?</script>\s*', '')
$index = [regex]::Replace($index, '(?s)<script>\s*document\.addEventListener\("DOMContentLoaded", \(\) => \{\s*const tabs = document\.querySelectorAll\("\.history-tab"\).*?</script>\s*', '')
$index = [regex]::Replace($index, '(?s)<script>\s*// v3\.4\.6 queue/history filter override.*?</script>\s*', '')
$index = $index -replace '<script src="/static/app.js\?v=\{\{ version \}\}"></script>', '<script src="/static/app.js?v={{ version }}-queuefix"></script>'
$index = $index -replace '<link rel="stylesheet" href="/static/style.css\?v=\{\{ version \}\}-cards-final">', '<link rel="stylesheet" href="/static/style.css?v={{ version }}-queuefix">'
Set-Content $indexPath $index -Encoding UTF8

# Add harmless CSS fallback marker/version comment.
$stylePath = Join-Path $ProjectRoot "app\static\style.css"
$style = Get-Content $stylePath -Raw -Encoding UTF8
$style += "`n/* v3.4.7.2 queue filters are controlled by app/static/app.js */`n"
Set-Content $stylePath $style -Encoding UTF8

Write-Host "Files patched to v3.4.7.2."

# Copy/build/deploy on NASDY because Windows does not have Docker.
Write-Host "Copying project to NASDY..."
scp -r . root@NASDY:/mnt/user/appdata/nasdy-media-linker-build

Write-Host "Building and deploying on NASDY..."
ssh root@NASDY 'cd /mnt/user/appdata/nasdy-media-linker-build && docker build -t nasdy-media-linker:latest . && docker rm -f nasdy-media-organizer 2>/dev/null || true; docker run -d --name nasdy-media-organizer --restart unless-stopped -p 8088:8088 --user 99:100 -e DATA_ROOT=/data -e DOWNLOADS_ROOT=/downloads -e MOVIES_ROOT=/media/movies -e TV_ROOT=/media/tv -v /mnt:/host_mnt -v /mnt/user/NASDY/downloads:/downloads -v /mnt/user/NASDY/media:/media -v /mnt/user/appdata/nasdy-media-organizer/data:/data nasdy-media-linker:latest && docker logs nasdy-media-organizer --tail=60'
